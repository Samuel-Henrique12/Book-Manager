...
import type { ItemEstante } from "@/lib/tipos";
import CapaLivro from "@/components/livro/CapaLivro";
import BarraProgresso from "@/components/ui/BarraProgresso";

// Livros em Andamento na Estante do Leitor
export default function ContinueLendo({ itens }: { itens: ItemEstante[] }) {
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 flex-col justify-between py-0.5">
            ...
            {item.totalPages ? (
              <BarraProgresso
                className="mt-3"
                paginaAtual={item.currentPage ?? 0}
                totalPaginas={item.totalPages}
              />
            ) : (
              <span className="mt-3 text-[12px] text-suave-2">Leitura em andamento</span>
            )}
          </div>
        </Link>
      ))}
